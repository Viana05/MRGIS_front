  ...
  try {
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      entity.point = {
        ...
        // 使用自定义图标
        // 如果需要使用图标，可以取消下面的注释并准备相应的图标文件
        // image: './geology_icon.png',
        // scale: 0.5
      };

      // 设置标签样式
      if (entity.properties && entity.properties.Image) {
        entity.label = {
          text: properties.Image.getValue(),
          font: "bold 14pt 微软雅黑",
          style: Cesium.LabelStyle.FILL_AND_OUTLINE,
          fillColor: Cesium.Color.fromCssColorString("#4A2C15"), // 深棕色
          outlineColor: Cesium.Color.WHITE,
          outlineWidth: 3,
      ...
      // 设置点击后的信息框内容
      if (entity.properties && entity.properties.Detail) {
        const image = properties.Image ? properties.Image.getValue() : '未命名地质点';
        const detail = properties.Detail ? properties.Detail.getValue() : '暂无详细描述';
        const latitude = properties.Latitude ? properties.Latitude.getValue() : '未知';
        const longitude = properties.Longitude ? properties.Longitude.getValue() : '未知';

        // 创建HTML内容用于信息框
        const description = `
          <div style="font-family: '微软雅黑', sans-serif; max-width: 400px;">
            ...
            <div style="background-color: #f9f9f9; padding: 10px; border-radius: 5px; border-left: 4px solid #FF5722;">
              <strong>地质特征描述:</strong>
              <p style="margin-top: 5px; line-height: 1.5;">${detail}</p>
            </div>
