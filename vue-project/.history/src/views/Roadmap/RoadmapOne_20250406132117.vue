...
onMounted(() => {
  ...
  // 显示实习路线
  const showPracticeRoute = () => {
      if (!viewer.value) return;

      // 如果路线已存在，先移除
      if (practiceRouteEntity.value) {
        viewer.value.entities.remove(practiceRouteEntity.value);
      }

      // 将坐标转换为Cesium Cartesian3数组
      const positions = practiceRouteData.coordinates.map(coord => 
        Cesium.Cartesian3.fromDegrees(coord[0], coord[1])
      );

      // 添加路线实体
      practiceRouteEntity.value = viewer.value.entities.add({
        name: '实习路线',
        polyline: {
          positions: positions,
          width: 5,
      ...
      // 添加起点和终点标记
      const startPoint = practiceRouteData.coordinates[0];
      const endPoint = practiceRouteData.coordinates[practiceRouteData.coordinates.length - 1];
      ...
      // 缩放到路线范围
      viewer.value.zoomTo(practiceRouteEntity.value);
    };
});
