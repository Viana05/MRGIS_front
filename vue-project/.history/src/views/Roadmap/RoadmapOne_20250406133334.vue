...
onMounted(() => {
  ...
  let walkerEntity = null;
  let isAnimating = false;
  let animationStopRequested = false;
  ...
  window.showPracticeRoute = function () {
    ...
    // 实习路线坐标数据
    const routeCoordinates = Cesium.GeoJsonDataSource.load("/json/routeOne.json");
  ...
  // 开始行走动画函数
  window.startWalkingAnimation = async function () {
    if (!viewer || !practiceRouteEntity || isAnimating) return;
    ...
    // 获取路线位置数组
    const positions = practiceRouteEntity.polyline.positions.getValue(
    ...
    // 添加行走者模型
    walkerEntity = viewer.entities.add({
      name: "行走者",
    ...
    // 设置相机跟随
    viewer.trackedEntity = walkerEntity;

    // 计算动画参数
    const animationDuration = totalDistance / 100; // 调整这个除数可以改变速度
    const startTime = viewer.clock.currentTime.secondsOfDay;
    const endTime = startTime + animationDuration;

    // 动画函数
    const animate = () => {
      if (animationStopRequested) return;
      ...
      while (
        ...
        accumulatedDistance + distances[segmentIndex] < targetDistance
      ) {
      ...
      const segmentProgress = segmentDistance / distances[segmentIndex];
      ...
      walkerEntity.position = currentPos;
      ...
      if (segmentIndex < positions.length - 1) {
        ...
        // 设置模型朝向
        walkerEntity.orientation = Cesium.Quaternion.fromHeadingPitchRoll(
  ...
  // 停止行走动画函数
  window.stopWalkingAnimation = function () {
    animationStopRequested = true;
    isAnimating = false;

    if (walkerEntity) {
      viewer.entities.remove(walkerEntity);
      walkerEntity = null;
    }
  ...
  <div class="cesium_map">
    ...
    <div class="floating-menu">
      <div class="menu-content">
        ...
        <el-row class="menu-row">
          <button onclick="showPracticeRoute()">显示实习路线</button>
          <button onclick="removePracticeRoute()">移除实习路线</button>
          <button onclick="startWalkingAnimation()">开始行走动画</button>
          <button onclick="stopWalkingAnimation()">停止动画</button>
          <button onclick="loadGeoJSONPoints()">加载地质点</button>
        </el-row>
      </div>
    </div>
  </div>
...
.menu-row button:nth-child(3) {
  background: #2196f3;
}

.menu-row button:nth-child(3):hover {
  background: #0b7dda;
}

.menu-row button:nth-child(4) {
  background: #f44336;
}

.menu-row button:nth-child(4):hover {
  background: #da190b;
}
