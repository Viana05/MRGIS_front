...
const loadGeoJSONPoints = async () => {
  try {
    ...
    // 计算所有实体的边界矩形
    const boundingSphere = dataSource.entities.computeBoundingSphere();
    const rectangle = Cesium.Rectangle.fromCartographicArray([
      Cesium.Cartographic.fromCartesian(boundingSphere.center),
      // 添加一些缓冲区域
    ...
    viewer.camera.flyTo({
      ...
      orientation: {
        heading: 0, // 正北方向
        pitch: -Cesium.Math.PI_OVER_TWO, // 垂直向下(-90度)
        roll: 0, // 无旋转
      },
      duration: 2, // 2秒动画过渡
      complete: function () {
        // 稍微拉远一点确保所有点可见
        viewer.camera.zoomOut(boundingSphere.radius * 0.5);
      },
    });
