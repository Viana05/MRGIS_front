      ...
      <div class="content-section rift-basin" id="rift-basin">
        ...
        <div class="section-content">
          <div class="intro-section">
            ...
            <p>
              裂陷盆地作为典型的伸展型盆地，是指那些由于岩石圈或地壳裂陷作用过程中形成的沉积盆地，在地质学中有着极其重要的意义。
            </p>
          </div>
          ...
          >
            ...
            <div class="content-card">
              ...
              <div class="definition-details">
                ...
                >
                  <h4>裂陷盆地基本特征</h4>
                  <div v-show="definitionDetails.basic" class="detail-content">
                    <p>
                      裂陷盆地是地壳表浅层的大型伸展构造,
          ...
          <!-- 分类1 -->
          <div v-show="activeBasinTab === 'classification1'" class="basin-classification1">
    <h3 class="subsection-title">（2）分类1</h3>
    
    <div class="content-card">
      <p>在板块的内部或不同板块边缘都可以有裂陷盆地的存在。它们的形成和演化直接或间接受到板块构造运动制约。</p>
      <p>因此，根据裂陷盆地形成演化过程与板块构造运动的关系, 可将该盆地分成如下图的四大类。</p>
      
      <div class="classification-section">
        <!-- 使用独立的展开/收起按钮 -->
        <div class="classification-group">
          <div class="group-header" @click="toggleClassificationGroup('intracontinental')">
            <h4>大陆板块内部的裂陷盆地</h4>
            <span class="toggle-icon">{{ classificationGroups.intracontinental ? '−' : '+' }}</span>
          </div>
          <div v-show="classificationGroups.intracontinental" class="group-details">
            <!-- 每个盆地类型使用独立卡片 -->
            <div class="basin-type-card" @click.stop="toggleBasinType('sag')">
              <div class="basin-header">
                <h5>伸展型拗陷盆地</h5>
                <span class="toggle-icon">{{ basinTypes.sag ? '−' : '+' }}</span>
              </div>
              <div v-show="basinTypes.sag" class="type-detail">
                <p>指地壳或岩石圈受引张减薄而形成的碟状坳陷。其形成的可能原因：岩石圈减薄未造成大规模破裂，或盆地位于大型坡坪式拆离断层的断坡部位。</p>
              </div>
            </div>
            
            <div class="basin-type-card" @click.stop="toggleBasinType('wide')">
              <div class="basin-header">
                <h5>陆内宽裂陷盆地</h5>
                <span class="toggle-icon">{{ basinTypes.wide ? '−' : '+' }}</span>
              </div>
              <div v-show="basinTypes.wide" class="type-detail">
                <p>是由一系列平行延伸或分叉、 斜交的地堑、 半地堑组成， 裂陷作用发生在很宽阔的地区。</p>
                <p>宽裂陷盆地可以被埋藏在后继的盆地沉积层之下，如渤海湾盆地，也可直接露出地表， 如美国西部盆地。</p>
              </div>
            </div>
            
            <div class="basin-type-card" @click.stop="toggleBasinType('narrow')">
              <div class="basin-header">
                <h5>陆内窄裂陷盆地</h5>
                <span class="toggle-icon">{{ basinTypes.narrow ? '−' : '+' }}</span>
              </div>
              <div v-show="basinTypes.narrow" class="type-detail">
                <p>裂陷作用发生在比较窄的范围内，地堑、半地堑呈链状、斜列式分布，构成平面上狭长沉降带，为典型大陆裂谷。如东非裂谷、 苏伊士湾等。</p>
              </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div class="content-card">
              ...
              <div class="fault-classification">
                <div class="fault-type" @click="toggleFaultType('geometry')">
                  <h4>正断层几何形态分类</h4>
                  <div v-show="faultTypes.geometry" class="fault-detail">
                    <p>
                ...
                <div class="fault-type" @click="toggleFaultType('structural')">
                  <h4>裂陷盆地构造样式分类</h4>
                  <div v-show="faultTypes.structural" class="fault-detail">
                    <p>
                ...
                >
                  <h4>裂陷盆地平面分布特征</h4>
                  <div v-show="faultTypes.distribution" class="fault-detail">
                    <p>
                      同一构造背景可形成多个裂陷盆地，
                ...
                <div class="fault-type" @click="toggleFaultType('transfer')">
                  <h4>构造变换带特征</h4>
                  <div v-show="faultTypes.transfer" class="fault-detail">
                    <p>
